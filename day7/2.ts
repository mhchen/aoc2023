...
const currentSize = directorySizeMap.get(root)!;
const needAdditionalSize = needSize - (totalSize - currentSize);
const bigEnough: number[] = [];
for (const size of directorySizeMap.values()) {
  if (size >= needAdditionalSize) {
    bigEnough.push(size);
  }
}

console.log(Math.min(...bigEnough));
